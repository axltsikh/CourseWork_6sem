...
class DatabaseHandler{
  ...
  static Future<Database> createDatabase() async{
    ...
    final Bufferdatabase = openDatabase(
        ...
        onCreate: (db,version) async{
          ...
          await db.execute('create table Project(id INTEGER PRIMARY KEY AUTOINCREMENT,title text,decription NVARCHAR(255),startDate date,endDate date,isDone INTEGER, changed INTEGER)');
  ...
  Future<void> uploadUnboundedParentSubTasks()async {
    ...
    await unboundedBuffer(subTasksBuffer, parents).then((value){
      parents = value;
      uploadUnboundedChildSubTasks(parents);
    });
  }
  ...
  Future<List<CustomProject>> getProjectsFromLocal() async{
    final db= await database;
    List<CustomProject> projectsBuffer = [];
    final List<Map<String,dynamic>> maps = await db.rawQuery("select Project.id,Project.title,Project.creatorID,"
        "Project.decription as Description,Project.endDate,Project.isDone,Project.startDate From Project "
        "inner join ProjectMember on Project.id = ProjectMember.projectID "
        "inner join OrganisationMember on OrganisationMember.id = ProjectMember.organisationMemberID "
        "where OrganisationMember.userID=? and OrganisationMember.deleted=0",[Utility.user.id]);
    maps.toList().forEach((element) {
      print("iteration");
      int valueBuffer = element.putIfAbsent("isDone", () => null);
      bool booleanBuffer;
  ...
  Future<List<CustomProjectMember>> getAllProjectMembers(int projectID) async{
    ...
    final List<Map<String,dynamic>> maps = await db.rawQuery("select ProjectMember.id,AppUser.username,OrganisationMember.id as organisationID,ProjectMember.deleted from ProjectMember inner join OrganisationMember on OrganisationMember.id = ProjectMember.organisationMemberID and OrganisationMember.deleted=0 inner join AppUser on AppUser.id = OrganisationMember.userID where projectID = ?"
        ,[projectID]);
    print(projectID);
    List<CustomProjectMember> buffer = [];
    maps.toList().forEach((element) {
      buffer.add(CustomProjectMember(element.putIfAbsent("id", () => null), element.putIfAbsent("username", () => null),
          element.putIfAbsent("organisationID", () => null), element.putIfAbsent("deleted", () => null)));
    });
    return buffer;
  }
